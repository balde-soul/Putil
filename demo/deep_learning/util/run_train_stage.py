...
logger):
    ...
    recorder.epoch = epoch if stage == util.RunStage.Train else recorder.epoch
    prefix = 'train' if stage == util.RunStage.Train else 'evaluate'
    loss_scalar_collection = util_with_args.ScalarCollection() if util_with_args.train_stage(args) else None
    indicator_scalar_collection = util_with_args.ScalarCollection() if util_with_args.train_stage(args) else None
    def accumulation_fix(index):
        return np.ceil(index / args.accumulation_time)
    with torch.no_grad() if stage == util.RunStage.Evaluate else torch.enable_grad() as t:
        backbone.train() if stage == util.RunStage.Train else backbone.eval()
        backend.train() if stage == util.RunStage.Train else backend.eval()
        decode.train() if stage == util.RunStage.Train else decode.eval()
        data_loader.sampler.set_epoch(epoch) if stage == util.RunStage.Train else None

        logger.debug('start to {} epoch'.format(prefix))
        for batch_idx, datas in enumerate(data_loader):
            datas = [data.cuda() for data in datas] if util_with_args.iscuda(args) else datas
            recorder.step += accumulation_fix(1) if stage == util.RunStage.Train else 0
            ...
            accumulated_opt.step(_loss, force_accumulation=None \
                if len(data_loader) % args.accumulation_time == 0 or \
                    len(data_loader) - batch_idx - 1 > len(data_loader) % args.accumulation_time \
                        else len(data_loader) % args.accumulation_time) if stage == util.RunStage.Train else None
            accumulated_opt.zero_grad()
            ## : run the backward
            #_loss.backward() if stage == util.RunStage.Train else None
            ## : do the optimize
            #optimizer.step() if stage == util.RunStage.Train else None
            ## do the training
            ...
            # while in util.RunStage.Train or util.RunStage.TrainEvaluate
            if (recorder.step % args.log_interval == 0 or recorder.step % args.summary_interval == 0) and stage == util.RunStage.Train:
                reduced_indicator_current = {util_with_args.ScalarCollection.generate_current_reduce_name(k): util_with_args.all_reduce(v, util_with_args.ScalarCollection.generate_current_reduce_name(k), hvd) \
                    for k, v in loss_scalar_collection.current_indicators.items()}
                reduced_loss_current = {util_with_args.ScalarCollection.generate_current_reduce_name(k): util_with_args.all_reduce(v, util_with_args.ScalarCollection.generate_current_reduce_name(k), hvd) \
                    for k, v in indicator_scalar_collection.current_indicators.items()}
                pass
                reduced_indicator_current.update(reduced_loss_current)
                util_with_args.scalar_log(logger, prefix, reduced_indicator_current, recorder, int(accumulation_fix(batch_idx)), \
                    int(accumulation_fix(len(data_loader)))) if recorder.step % args.log_interval == 0 and hvd.rank() == 0 and stage == util.RunStage.Train else None
            if recorder.step % args.summary_interval == 0 and stage == util.RunStage.Train:
        ...
        reduced_indicator_epoch_average = {util_with_args.ScalarCollection.generate_epoch_average_reduce_name(k): util_with_args.all_reduce(v, util_with_args.ScalarCollection.generate_epoch_average_reduce_name(k), hvd) \
            for k, v in indicator_scalar_collection.epoch_average.items()}
        reduced_loss_epoch_average = {util_with_args.ScalarCollection.generate_epoch_average_reduce_name(k): util_with_args.all_reduce(v, util_with_args.ScalarCollection.generate_epoch_average_reduce_name(k), hvd) \
        ...
        if hvd.rank() == 0:
            ...
            util_with_args.scalar_log(logger, prefix, reduced_indicator_epoch_average, recorder, None, None)
