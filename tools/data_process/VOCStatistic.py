...
import pandas as pd
class StatisticFileToolset:
    def __init__(self, *args, **kwargs):
        pass

    def _formate_file_to_df(self, df):
        def format_func(x):
    ...
    def read_statistic_file(self, statistic_file):
        df = pd.read_csv(statistic_file, index_col=0)
        df = self._formate_file_to_df(df)
        return df
    
    ##@brief 生成type_set列表
    # @note
    # @param[in] formated df
    # @return 
    # @time 2023-04-03
    # @author cjh
    def get_type_set(self, df):
        return list(set(itertools.chain(*tuple(df['type_set']))))

    def extract_type_data(self, df, type_name):
        return df[df[type_name].isna().eq(False)]

    def _formate_df_to_file(self, df):
        pass

    def save_statistic_file(self, df, statistic_file):
        pass
