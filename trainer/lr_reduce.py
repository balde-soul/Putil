...
class LrReduce:
    ...
    def generate_args(parser):
        parser.add_argument('--lr_reduce_init_lr', dest='LrReduceInitLr', type=float, action='store', default=0.001, 
        help='the init lr')
        parser.add_argument('--lr_reduce_lr_factor', dest='LrReduceLrFactor', type=float, action='store', default=0.1, 
        help='the reduce factor for lr reducion')
        parser.add_argument('--lr_reduce_lr_epsilon', dest='LrReduceLrEpsilon', type=float, action='store', default=0.01, 
        help='the threshold which decide the reducion is triggered or not')
        parser.add_argument('--lr_reduce_lr_patience', dest='LrReduceLrPatience', type=int, action='store', default=3, 
        help='while reducion is triggered in LrReduceLrPatience time, the lr whould be reduce')
        parser.add_argument('--lr_reduce_lr_cool_down', dest='LrReduceLrCoolDown', type=int, action='store', default=2, 
        help='the epoch in which the reducion trigger would not be count')
        parser.add_argument('--lr_reduce_lr_min', dest='LrReduceLrMin', type=float, action='store', default=0.0000001, 
        help='the minimum of the lr')
        ...
        the reducion would be triggered')
        pass
