    ...
    def _check_keys(self):
        for key in self._param.keys():
            if key in self._default.keys():
                ...
                RegularizeExtractLogger.warning(
                    'key: {0} is illegal, keys must in {1}'.format(key, self._default.keys()))
                self._warn_up = True
        pass
...
class conv_extract(ParamProbe):
    def __init__(self, param):
        ...
        ParamProbe.__init__(self, _default_param, param)
        pass
