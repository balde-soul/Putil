...
options.add_argument('--set_rate', dest='SetRate', type=float, nargs='+', help='指定分割比例')
options.add_argument('--set_name', dest='SetName', type=str, nargs='+', help='指定分割集合名称')
#options.add_argument('--class_filter_and_onehot', dest='ClassFilterAndOnehot', type=str, action='append', help='指定类别过滤器，以及使用该过滤器进行独热码赋值，labelme标记并不指定对应类别的独热码')
...
options.add_argument('--class_reflect', dest='ClassReflect', type=str, nargs='+', help='目标类别重映射,用于多种标注类别混合成一种,格式为--class_reflect A,B C,D')
...
if args.YoloLabelSaveTo == '':
    ...
    formated_save_to = os.path.join(args.YoloLabelSaveTo, 'labels')
...
class_reflect = {c: cs_index for cs_index, cs in enumerate([cr.split(',') for cr in args.ClassReflect]) for c in cs} if args.ClassReflect is not None else None
# 检查类型映射是否满足独热码
if class_reflect is not None:
...
def convert_annotation(xml_id, classes):
    ...
    for obj in root.iter('object'):
        ...
        if (object_type not in classes or int(difficult)==1) and object_type not in class_reflect.keys():
            continue
        cls_id = classes.index(object_type) if class_reflect is None else class_reflect[object_type]
...
for index, sn in zip(set_index, set_name):
    ...
    with open(os.path.join(split_save_to, '{0}.txt'.format(sn)), 'w') as set_file:
        for xml_id in xml_ids:
            try:
                ...
                print(ex.args)
                continue
            set_file.write('{0}\n'.format(os.path.join(image_root, image_name)))  # 修改路径（最好使用全路径）
            pass
        pass
    pass
